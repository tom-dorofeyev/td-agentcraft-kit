...
- `## Success Criteria` — checkbox list, measurable from a user/business perspective. Each item must be independently verifiable by QA without guessing.
- `## Scope`
...
- **Independent verifiability**: a slice must be reviewable and QA-verifiable without waiting for any other slice to be complete. If it can't, split it further.
...
Acceptance criteria are the contract between Product and QA. They must be:

- **Objectively testable**: a QA engineer must be able to verify each criterion from a user perspective without asking for clarification.
...
## When Reviewing a Spec (Team Leader, QA, Architect)

A spec is incomplete and must be returned if:
- Any required section is missing or empty
- A success criterion is subjective or unverifiable by QA
